...
library("ggplot2")
library("ggalluvial")
a<- list_all_PEMs <- setwd("D:/dev/rust/proteomics-rs/data/benchmarks/phospho_evidence_matrices_list/results_eyers_correct_list/twophospho_int_count/")
...
# main_dir [1], main_dir [2] 3phospho count and sum
# main_dir [4], main_dir [5] monophospho count and sum
# main_dir [6], main_dir [7] twophospho count and sum
...
for(i in 1:length(aa)) {
  ...
  # Select a row (spectrum) that is given by max values 
  data_int_count_max <- as.data.frame(int_count_wo_spec) %>% slice(which.max(rowSums(int_count_wo_spec)))
  ...
  rownames(data_int_count_max) <- paste(intentisty_mod[1], list_all_pep[i], sep = "-")
  
  # # # Change directory to intensity summation
  set_dir <- assign(paste0("set_dir", i), setwd((paste0(list_all_PEMs,"/",main_dir[7],"/"))))
  set_dir <- assign(paste0("set_dir", i), setwd((paste0(list_all_PEMs,"/",main_dir[7],"/"))))
  ...
  # Select a row (spectrum) that is given by max values 
  data_int_sum_max <- as.data.frame(int_sum_wo_spec) %>% slice(which.max(rowSums(int_sum_wo_spec)))
  #data_int_sum_agg <- t(as.data.frame(colSums(int_sum_wo_spec)))
  
  # Indicate its peptide info and type of phospho evidence calculation (either count or sum)
  #rownames(data_int_sum_agg) <- paste(list_all_pep[i], sep = "_")
  
  rownames(data_int_sum_max) <- paste(intentisty_mod[2], list_all_pep[i], sep = "-")
  
  int_both <- rbind(data_int_count_max,data_int_sum_max)
...
# Write output as tsv
setwd("D:/dev/rust/proteomics-rs/data/benchmarks/phospho_evidence_matrices_list/combined_result_of_PEMs/results_eyers_correct_list")

write.table(tp_final, file = "TP_pos_values_Eyers_max_values_twophosphate.tsv", sep = "\t")
write.table(fp_final, file = "FP_pos_values_Eyers_max_values_twophosphate.tsv", sep = "\t")
...
# To reduce dimension of individual Ps and NPs
# P's (True Positive)
...
# Select a row that has the highest value
#tp_final_log_P_max1 <- as.data.frame(tp_final_log_Ps) %>% slice(which.max(rowSums(tp_final_log_Ps)))
tp_final_log_P_max1 <- apply(tp_final_log_Ps, 1, max)
# NP's (True Positive)
tp_final_log_NPs <-as.data.frame(tp_final_log) %>% select(matches("NP"))

# Row summation
tp_final_log_NPs_rowsum1 <- as.data.frame(rowSums(tp_final_log_NPs))
# Select a row that has the highest value
#tp_final_log_NPs_max1 <- as.data.frame(tp_final_log_NPs) %>% slice(which.max(rowSums(tp_final_log_NPs)))
tp_final_log_NPs_max1 <- apply(tp_final_log_NPs, 1, max)
# Collect P's and NP's in one object (True Positive)
# Row sum
tp_final_log_rowsum1 <- cbind(tp_final_log_P_rowsum1,tp_final_log_NPs_rowsum1)
colnames(tp_final_log_rowsum1) <- c("P","NP")
# Max values
tp_final_log_max1 <- cbind(tp_final_log_P_max1,tp_final_log_NPs_max1)
colnames(tp_final_log_max1) <- c("P","NP")
...
# Select a row that has the highest value
#fp_final_log_P_max1 <- as.data.frame(fp_final_log_Ps) %>% slice(which.max(rowSums(fp_final_log_Ps)))
fp_final_log_P_max1 <- apply(fp_final_log_Ps, 1, max)
# NP's (False Positive)
...
# Select a row that has the highest value
#fp_final_log_NPs_max1 <- as.data.frame(fp_final_log_NPs) %>% slice(which.max(rowSums(fp_final_log_NPs)))
fp_final_log_NPs_max1 <- apply(fp_final_log_NPs, 1, max)
# Collect P's and NP's in one object (False Positive)
# Row sum
fp_final_log_rowsum1 <- cbind(fp_final_log_P_rowsum1,fp_final_log_NP_rowsum1)
colnames(fp_final_log_rowsum1) <- c("P","NP")

# Max values
fp_final_log_max1 <- cbind(fp_final_log_P_max1,fp_final_log_NP_rowsum1)
colnames(fp_final_log_max1) <- c("P","NP")
...
# Change object name either "tp_final_log_rowsum1' or 'tp_final_log_rowmax1'
tp_final_int_mod <- cbind(tp_final_log_max1, sep_int_mod,"true_positive")

fp_final_int_mod <- cbind(fp_final_log_max1, sep_int_mod,"false_positive")
...
par(mar = c(5, 4, 4, 8),                                
    xpd = TRUE)
plot(comb_tp_fp_mono$TP, xlim=c(0,470), ylim=c(0,10),pch = 19, xlab = "indeces of each score", 
     ylab="scores", main = "Distribution of TP and TNP values", sub = "mono and two phosphate (max selection)")
par(new=TRUE)
plot(comb_tp_fp_two$TP, col="red",xlim=c(0,470), ylim=c(0,10),pch = 19,xlab = "", ylab="")
par(new=TRUE)
plot(comb_tp_fp_two$TNP, col="blue",xlim=c(0,470), ylim=c(0,10),pch = 19,xlab = "", ylab="")
par(new=TRUE)
plot(comb_tp_fp_mono$TNP, col="#69b3a2", xlim=c(0,470), ylim=c(0,10),pch = 19,xlab = "", ylab="")
coord <- par("usr")
...
par(mar = c(5, 4, 4, 8),                                
    xpd = TRUE)
plot(comb_tp_fp_mono$FP, xlim=c(0,470), ylim=c(0,110),pch = 19, xlab = "indeces of each score", 
     ylab="scores", main = "Distribution of FP and FNP values", sub = "mono and two phosphate (max selection)")
#axis(side = 2, at=seq(0,110,10))
par(new=TRUE)
plot(comb_tp_fp_two$FP, col="red",xlim=c(0,470), ylim=c(0,110),pch = 19,xlab = "", ylab="")
#axis(side = 2, at=seq(1,110,10))
par(new=TRUE)
plot(comb_tp_fp_two$FNP, col="blue",xlim=c(0,470), ylim=c(0,110),pch = 19,xlab = "", ylab="")
#axis(side = 2, at=seq(1,110,10))
par(new=TRUE)
plot(comb_tp_fp_mono$FNP, col="#69b3a2", xlim=c(0,470), ylim=c(0,110),pch = 19,xlab = "", ylab="")
#axis(side = 2, at=seq(1,110,10))
coord <- par("usr")
legend(x = coord[2] * 1.05, y = coord[4],
...
ggplot(melted, aes(x=melted[,"1:dimmelt[1]"], y=value, alpha=melted[,"phospho_state"] )) + geom_point(size=2,shape=19, color=("black")) + 
  theme_ipsum() +
...
# Data reshaping to become more suitable for ggplot representation

comb_tnps_tps_fps <- cbind(comb_tp_fp_mono$TP,comb_tp_fp_two$TP, comb_tp_fp_mono$TNP, 
                           comb_tp_fp_two$TNP, comb_tp_fp_mono$FP, comb_tp_fp_mono$FNP, 
                           comb_tp_fp_two$FP,comb_tp_fp_two$FNP)

dim_comb_tnps_tps_fps <- dim(comb_tnps_tps_fps)[1]

comb_tnps_tps_fps[96:dim_comb_tnps_tps_fps[1],2] <- 0
comb_tnps_tps_fps[96:dim_comb_tnps_tps_fps[1],4] <- 0
comb_tnps_tps_fps[96:dim_comb_tnps_tps_fps[1],7:8] <- 0
colnames(comb_tnps_tps_fps) <- c("TP_mono","TP_two","TNP_mono","TNP_two", "FP_mono","FNP_mono","FP_two","FNP_two")
...
ggplot(as.data.frame(comb_tnps_tps_fps), aes(x=FP_mono, y=FNP_mono)) + 
  geom_point( color="#69b3a2") +
  geom_smooth(method=lm , color="red", fill="#69b3a2", se=TRUE) +
  theme_ipsum() +labs(x="Mono FP of Phospho Evidence scores",y=" Mono FNP of Phospho Evidence scores", 
                      title="Distribution of FP and FNP scores on Eyers dataset",subtitle="monophosphate")

# Special dimension of all figures is 1100 x 630 except figure that were generated by plot() function
...
### Extraction of 1-5 pool.csv

setwd("D:/dev/rust/proteomics-rs/data/benchmarks/revision1/InputData/FerriesEtAl/PeptidePools")
pool_list <- list.files(setwd("D:/dev/rust/proteomics-rs/data/benchmarks/revision1/InputData/FerriesEtAl/PeptidePools"))

for(i in 1:length(pool_list)) {
  
tmp <- assign(paste0("pool", i),read.csv(paste0(pool_list[i]),header=TRUE))
tmp2 <- assign(paste0("pool_seq_tpos", i),tmp[,c(1,4,5,8)])
assign(paste0("pool_seq_tpos_v", i),as.data.frame(paste("pool", i, tmp2$Phospopeptide.sequence,tmp2$modified.position.in.peptide, sep = "_")))
